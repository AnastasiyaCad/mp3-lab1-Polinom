...
double TPostfix::Calculate22(TUnArray _table)
{
	TStack<int>stack(postfix.length());
	string tmp;
	int tmp1 = 0;
	int tmp2 = 0;
	for (int i = 0; i < postfix.length(); i++)
	{
		...
		else
		{
			...
			case '+': {
				tmp2 = stack.Pop();
				tmp1 = stack.Pop();
				TPolinom t1 = _table.ToFindUnArr2(tmp1);
				TPolinom t2 = _table.ToFindUnArr2(tmp2);
				TPolinom t3 = t1 + t2;
				int a = _table.ToFindUnArr(tmp1);
				_table.Polinom[a].data = t1+t2;
				_table.ToDeleteUnArr(tmp2);
				stack.Push(tmp1);
				break; }
			case '-': {
				tmp1 = stack.Pop();
